...
class ProductProvider extends ChangeNotifier {
  ...
  void setCartVariationIndex(int index, int i, Product product, bool isMultiSelect) {
    if(!isMultiSelect) {
      ...
      if(!_selectedVariations[index][i] && selectedVariationLength(_selectedVariations, index) >= product.variations[index].max) {
        ...
        } ${getTranslated('is', Get.context)} ${product.variations[index].max}', Get.context, isToast: true);
  ...
  int setExistInCart(Product product, {bool notify = true}) {
    ...
    if(_cartIndex != -1) {
      ...
      _addOnQtyList = [];
      List<int> _addOnIdList = [];
      cartProvider.cartList[_cartIndex].addOnIds.forEach((addOnId) => _addOnIdList.add(addOnId.id));
      product.addOns.forEach((addOn) {
        if(_addOnIdList.contains(addOn.id)) {
          _addOnActiveList.add(true);
          _addOnQtyList.add(cartProvider.cartList[_cartIndex].addOnIds[_addOnIdList.indexOf(addOn.id)].quantity);
        }else {
          _addOnActiveList.add(false);
          _addOnQtyList.add(1);
        }
